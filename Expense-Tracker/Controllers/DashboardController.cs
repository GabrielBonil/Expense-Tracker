...
namespace Expense_Tracker.Controllers
{
    public class DashboardController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            //Total Expense
            int TotalExpense = SelectedTransactions.Where(e => e.Category.Type == "Expense").Sum(j => j.Amount);
            ...
            ViewBag.DoughnutChartData = SelectedTransactions.Where(i => i.Category.Type == "Expense").GroupBy(j => j.CategoryId)
                .Select(k => new 
                { 
                    categoryTitleWithIcon = k.First().Category.TitleWithIcon, //k.First().Category.Icon + " " + k.First().Category.Title,
                    amount = k.Sum(j => j.Amount), 
                    formattedAmount = k.Sum(j => j.Amount).ToString("C0"),
                }).OrderByDescending(l => l.amount).ToList();

            //Span Chart - Income vs Expense
            //Income
            List<SplineChartData> IncomeSummary = SelectedTransactions.Where(i => i.Category.Type == "Income").GroupBy(j => j.Date)
            ...
            //Expense
            List<SplineChartData> ExpenseSummary = SelectedTransactions.Where(e => e.Category.Type == "Expense").GroupBy(j => j.Date)
                .Select(k => new SplineChartData()
                {
                    day = k.First().Date.ToString("dd-MMM"),
